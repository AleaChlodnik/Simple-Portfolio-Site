...
//       setMaxNameLength(length); // Update the maxNameLength whenever repos change
//     }
//   }, [repos]);
...
//             {/* Pie Chart Section */}
//             <div className="w-fit bg-yellow-100">
//               {error ? (
...
//             {/* Projects Section */}
//             <div className="flex flex-col items-end gap-5 w-full h-full pr-5 bg-blue-300">
...
//                   {repos.map((repo) => (
//                     const width = `${maxNameLength * 10}px`; // Adjust width based on the longest project name
//                     return (
//                     <Link key={repo.id} href={repo.html_url}>
//                       <div className="rounded-full shadow-2xl text-4xl text-white text-center font-semibold bg-[#b78a6a] h-fit" style={{ width }}>
//                         {repo.name}
//                       </div>
//                     </Link>
//                     );
...
export default function Portfolio() {
  ...
  useEffect(() => {
    if (repos.length > 0) {
      const length = Math.max(...repos.map((repo) => repo.name.length));
      setMaxNameLength(length); // Update maxNameLength
      console.log("Max name length:", length); // Debugging line
    }
  }, [repos]);
          ...
          <div className="flex flex-row items-center w-full">
            ...
            <div className="flex flex-col items-end gap-5 w-full h-full pr-5 bg-blue-300">
              ...
              {error ? (
                ...
                <div className="flex flex-wrap justify-around gap-4">
                  {repos.map((repo) => {
                    const width = `${maxNameLength}vw`; // Apply the calculated width
                    return (
                      <Link key={repo.id} href={repo.html_url}>
                        <div className="rounded-full shadow-2xl text-4xl text-white text-center font-semibold bg-[#b78a6a] h-fit" style={{ width }}>
                          {repo.name}
